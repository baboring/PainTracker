...
namespace HC {
	...
	public class R {
				...
				if(null != mapCSVTable) { 
					if (mapCSVTable.TryGetValue("SystemMsg", out table)) {
						...
						foreach (var rec in table.tRecords) {
							R.SystemMsg newOne = new R.SystemMsg() {
								id = rec.tContents[0].ToEnum<SystemMsg.eKey>(),
			...
			try {
				...
				if(null != mapCSVTable) { 
					if (mapCSVTable.TryGetValue("MessageBox", out table)) {
						...
						foreach (var rec in table.tRecords) {
							R.MessageBox newOne = new R.MessageBox() {
								ClassName = rec.tContents[0],
								Type = rec.tContents[1].ToEnum<DefaultValue.eKey>(),
								Value = rec.tContents[2],
								StrValue = rec.tContents[3],
							};
							dic.Add(newOne.ClassName, newOne);
						}
					}
				}
			}
		...
		public static SystemMsg GetSystemMsg(SystemMsg.eKey key) {
			if (_instance._data_SystemMsg.ContainsKey(key))
				return _instance._data_SystemMsg[key];
			return null;
		}
		...
		public class SystemMsg {
			...
			public enum eKey : int {
				...
				TitleWarning,
				MsgNotRegisterPopup,
				CantUseDuplicatingPopup,
				FoundIllegalApp,
				FoundIllegalSignature,
			}
		...
		// -----------------------------------------------
		public static MessageBox GetMessageBox(string key) {
			if (_instance._data_MessageBox.ContainsKey(key))
				return _instance._data_MessageBox[key];
			return null;
		}
		public static bool TryGetMessageBox(string key,out MessageBox val) {
			return (_instance._data_MessageBox.TryGetValue(key, out val));
		}
		...
		public class MessageBox {
			...
			public DefaultValue.eKey Type;
			public int Value;
			public string StrValue;

		}

		private Dictionary<string, MessageBox> _data_MessageBox;
		public static Dictionary<string, MessageBox> AllMessageBox {
			get { return _instance._data_MessageBox; }
		}
